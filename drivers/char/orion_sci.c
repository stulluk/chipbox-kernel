...
MODULE_DESCRIPTION("Celestial Semiconductor Smart card driver");
MODULE_LICENSE("GPL");
...
#define SCI_BASE_ADDR1		0x101f3000
#define SCI_BASE_SIZE		0x1000

#define SCI_IRQ				14
#define SCI_MAX_FIFOSIZE    32

#define SCI_REF_CLK  (CPU_FREQ/4) /* 55MHz for CSM1200, 54MHz for CSM1201 */
#define MHZ	1000000
...
/* 
  * the following 3 registers is not used for orion_1.4 chip 
 */
...
static void _sci_set_sc_clock(int ch, unsigned int f) /* f is smart card clock (Hz)*/
{
	...
	sci_clkicc= (u32)(SCI_REF_CLK*MHZ*10/(f * 2))-10;		/* "*10","-10"  are used for rounding */
...
static unsigned int _sci_get_sc_clock(int ch) /* return value is smart card clock (Hz)*/
{
	...
	ret = (unsigned int)(SCI_REF_CLK*MHZ/ ((sci_clkicc +1)*2));
	return ret;
}
			   ...
			   unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case SCI_IOC_GET_RXRDYSIZE:
			tmp_val = LEFT(rq);
			if (copy_to_user((void*)arg, &tmp_val, sizeof(tmp_val))) 
				ret = -EFAULT;
			break;
...
static irqreturn_t sci_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
 ...
 */
int __init orion_sci_init(void)
{
	int error = 0;
    ...
    request_mem_region(SCI_BASE_ADDR0, SCI_BASE_SIZE, "ORION SCI");
	sci_base = (volatile unsigned short *)ioremap(SCI_BASE_ADDR0, SCI_BASE_SIZE);
	...
	if ((error = request_irq(SCI_IRQ, sci_interrupt, SA_INTERRUPT, "smartcard", NULL)) != 0){
	...
	printk( " Smart card: base address %08x map to %08x\n", SCI_BASE_ADDR0, (unsigned int)sci_base);

	return error;
}

static void __exit orion_sci_exit(void)
{
	/* do something here, ... */
	//iounmap(sci_base);
	release_mem_region(SCI_BASE_ADDR0, SCI_BASE_SIZE);
	misc_deregister(&sci_dev);
	remove_proc_entry("sci_io", &proc_root);
	free_irq(SCI_IRQ, NULL);
	
}

module_init(orion_sci_init);
module_exit(orion_sci_exit);
