...
#define ORION_WDT_BASE 		0x101e1000
#define ORION_WDT_SIZE 		0x100
#define ORION_WDT_IRQ		0
#define ORION_WDT_CLOSE_MAGIC	(0xbbffbbff)
...
static int orion_wdt_release(struct inode *inode, struct file *file)
{
	...
	if (expect_close == ORION_WDT_CLOSE_MAGIC) {
		wdt_base[WDT_CR] = 0x0;
	} else {
...
static ssize_t orion_wdt_write(struct file *file, const char *data, size_t len, loff_t *ppos)
{
	if (len) {
		if (!nowayout) {
			size_t i;
			...
			for (i = 0; i != len; i++) {
				...
				if (get_user(c, data + i))
					return -EFAULT;
				if (c == 'V')
					expect_close = ORION_WDT_CLOSE_MAGIC;
			}
		}
		/* Refresh timer. */
		wdt_base[WDT_CRR] = 0x76;
...
static int orion_wdt_ioctl(struct inode *inode, struct file *file,
...
static struct file_operations orion_wdt_fops =
{
	.owner		= THIS_MODULE,
	.llseek		= no_llseek,
	.write		= orion_wdt_write,
	.ioctl		= orion_wdt_ioctl,
	.open		= orion_wdt_open,
...
static int __init orion_wdt_init(void)
{
	...
	if (ret) {
		...
		printk("Orion Watchdog Timer: timer margin %d sec\n",
		       (margin == WDT_1SEC ) ? 1 :
		       (margin == WDT_2SEC ) ? 2 :
		       (margin == WDT_5SEC ) ? 5 : 
		       (margin == WDT_10SEC) ? 10:
	...
	wdt_base = (u32 *)ioremap(ORION_WDT_BASE, ORION_WDT_SIZE);
	if (!wdt_base) {
