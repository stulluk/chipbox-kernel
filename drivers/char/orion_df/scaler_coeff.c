...
int mpa_mul_q30(int x1, int x2)	
{
    ...
    val = (int)(((long long)x1 * (long long)x2) >> 32);
...
int mpa_mul_q28(int x1, int x2)	
{
    ...
    val = (int)(((long long)x1 * (long long)x2) >> 34);
...
//************************************************************//
int Linear (int s, unsigned int m) // sample position of the filter
{
	int val = 0;
	
	s = ABS(s);
	if (s <= 0x10000000) // s <= 1 (Q0.28)
		val= 0x10000000 - s;
	else
		val = 0;
