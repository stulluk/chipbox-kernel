...
static s32 IsRectValid(CSGFXOBJ_Bitmap_t *Img, CSGFXOBJ_Rectangle_t *Rect) //Boolean
{
...
static s32 CmpRect( CSGFXOBJ_Rectangle_t *Rect0, CSGFXOBJ_Rectangle_t *Rect1) 
{
...
/* valid returns 1, invalid returns 0 */
static s32 IsColorTypeValid( CSGFXOBJ_ColorType_t ColorType)
{
	switch (ColorType)
	{
		case CSGFXOBJ_COLOR_TYPE_RGB565:
		case CSGFXOBJ_COLOR_TYPE_ARGB4444:
		case CSGFXOBJ_COLOR_TYPE_A0:
		case CSGFXOBJ_COLOR_TYPE_CLUT8:
		case CSGFXOBJ_COLOR_TYPE_CLUT4:
		//case CSGFXOBJ_COLOR_TYPE_ARGB1555:
			return 1;
		default:
			return 0;
	}
...
)
{
	...
	//S0 Control
		if (BlitSrcId == 0)
		{
			...
			hBlitDC->S0ColorType  = CSGFXOBJ_COLOR_TYPE_A0;  //Must

			hBlitDC->S0DefaultColor.Type =  CSGFXOBJ_COLOR_TYPE_ARGB8888;     //ARGB8888 Format
	...
	//S1 Control
		if (BlitSrcId == 1)
		{
			...
			hBlitDC->S1ColorType  = CSGFXOBJ_COLOR_TYPE_A0;  //Must

			hBlitDC->S1DefaultColor.Type =  CSGFXOBJ_COLOR_TYPE_ARGB8888;     //ARGB8888 Format
...
*--------------------------------------------------*/
		...
		hBlitDC->DByteLittleNotBigEndian = DesImg->ByteLittleNotBigEndian;
		hBlitDC->DNibbleLittleNotBigEndian = DesImg->NibbleLittleNotBigEndian;	
		hBlitDC->DTwoBytesLittleNotBigEndian = DesImg->TwoBytesLittleNotBigEndian;
	...
	//S0 Control
		if (BlitSrcId == 0)
		{
			...
			*/
			...
			hBlitDC->S0ByteLittleNotBigEndian = SrcImg->ByteLittleNotBigEndian;
			hBlitDC->S0NibbleLittleNotBigEndian = SrcImg->NibbleLittleNotBigEndian;	
			hBlitDC->S0TwoBytesLittleNotBigEndian = SrcImg->TwoBytesLittleNotBigEndian;

			hBlitDC->S0ClutEnable     = (SrcImg->ColorType == CSGFXOBJ_COLOR_TYPE_CLUT4 ||
				                        SrcImg->ColorType == CSGFXOBJ_COLOR_TYPE_CLUT8 );
			hBlitDC->S0ColorKeyRGB.RGBEnable = FALSE;
			/*
			hBlitDC->S0ColorKeyMin;
			hBlitDC->S0ColorKeyMax;
	...
	//S1 Control
		if (BlitSrcId == 1)
		{
			...
			*/
			...
			hBlitDC->S1ByteLittleNotBigEndian = SrcImg->ByteLittleNotBigEndian;
			hBlitDC->S1NibbleLittleNotBigEndian = SrcImg->NibbleLittleNotBigEndian;	
			hBlitDC->S1TwoBytesLittleNotBigEndian = SrcImg->TwoBytesLittleNotBigEndian;

			hBlitDC->S1ClutEnable     = (SrcImg->ColorType == CSGFXOBJ_COLOR_TYPE_CLUT4 ||
				                        SrcImg->ColorType == CSGFXOBJ_COLOR_TYPE_CLUT8 );
			hBlitDC->S1ColorKeyRGB.RGBEnable = FALSE;
			/*
			hBlitDC->S1ColorKeyMin;
...
{
    	CSGFXOBJ_Bitmap_t     	*SrcImg = &SrcPara->Data.Bitmap;
    	CSGFXOBJ_Rectangle_t  	*SrcRect = &SrcPara->Rectangle; 
    	CSGFXOBJ_Bitmap_t    	*DesImg = &DesPara->Bitmap; 
   	CSGFXOBJ_Rectangle_t 	*DesRect = &DesPara->Rectangle;
	...
	else
	{
		...
		{	//S0 Control
			...
			hBlitDC->S0DefaultColor.Type = CSGFXOBJ_COLOR_TYPE_ARGB8888;
...
)
{
	...
	{
		int Err = -EINVAL;
		CSGFXOBJ_Bitmap_t *S0Img = NULL, *S1Img = NULL, *DImg = NULL;
	...
	//S0 Control
		{
			...
			*/
			...
			hBlitDC->S0ByteLittleNotBigEndian   = S0Img->ByteLittleNotBigEndian;
			hBlitDC->S0NibbleLittleNotBigEndian = S0Img->NibbleLittleNotBigEndian;
			hBlitDC->S0TwoBytesLittleNotBigEndian  = S0Img->TwoBytesLittleNotBigEndian;

			hBlitDC->S0ClutEnable     = (S0Img->ColorType == CSGFXOBJ_COLOR_TYPE_CLUT4 || 
				                        S0Img->ColorType == CSGFXOBJ_COLOR_TYPE_CLUT8 );
			//hBlitDC->S0ColorKeyEnable = Src0Para->ColorKeyRGB.RGBEnable;
			//memcpy(&hBlitDC->S0ColorKeyMin, &Src0Para->MinColor, sizeof(CSGFXOBJ_Corlor_t));
			//memcpy(&hBlitDC->S0ColorKeyMax, &Src0Para->MaxColor, sizeof(CSGFXOBJ_Corlor_t));
			memcpy(&hBlitDC->S0ColorKeyRGB, &Src0Para->ColorKeyRGB, sizeof(CSGFXOBJ_ColorKeyRGB_t));
		}
	//S1 Control
		{
			hBlitDC->S1Enable       = TRUE;
			...
			hBlitDC->S1DefaultColor.Type = CSGFXOBJ_COLOR_TYPE_ARGB8888;
			...
			*/
			...
			hBlitDC->S1ByteLittleNotBigEndian   = S1Img->ByteLittleNotBigEndian;
			hBlitDC->S1NibbleLittleNotBigEndian = S1Img->NibbleLittleNotBigEndian;
			hBlitDC->S1TwoBytesLittleNotBigEndian  = S1Img->TwoBytesLittleNotBigEndian;

			hBlitDC->S1ClutEnable     = (S1Img->ColorType == CSGFXOBJ_COLOR_TYPE_CLUT4 || 
				                        S1Img->ColorType == CSGFXOBJ_COLOR_TYPE_CLUT8 );
			//hBlitDC->S1ColorKeyEnable = Src1Para->ColorKeyEnable;
			//memcpy(&hBlitDC->S1ColorKeyMin, &Src1Para->MinColor, sizeof(CSGFXOBJ_Corlor_t));
			//memcpy(&hBlitDC->S1ColorKeyMax, &Src1Para->MaxColor, sizeof(CSGFXOBJ_Corlor_t));
			memcpy(&hBlitDC->S1ColorKeyRGB, &Src1Para->ColorKeyRGB, sizeof(CSGFXOBJ_ColorKeyRGB_t));
		}
	    //ROP and Compositor
		...
		hBlitDC->DByteLittleNotBigEndian = DImg->ByteLittleNotBigEndian;
		hBlitDC->DNibbleLittleNotBigEndian = DImg->NibbleLittleNotBigEndian;	
		hBlitDC->DTwoBytesLittleNotBigEndian = DImg->TwoBytesLittleNotBigEndian;
...
)
{
	...
	{
		int Err = -EINVAL;
		CSGFXOBJ_Bitmap_t *S0Img = NULL, *DImg = NULL;
		S0Img = &Src0Para->Data.Bitmap;
		DImg  = &DesPara->Bitmap;
	//S0 Control
		{
	...
	//S1 Control
		{
			...
			hBlitDC->S1FetchDram    = FALSE;          // 1 Fetch, 0 do not Fetch Using Default Color in ARGB
			hBlitDC->S1ColorType  = CSGFXOBJ_COLOR_TYPE_A0;                                     // if S1 Using Default Color then S1ColorType should be ARGB565 or A0
			memcpy(&hBlitDC->S0DefaultColor, &Src1Para->Data.Color, sizeof(CSGFXOBJ_Color_t));
			hBlitDC->S1ClutEnable     = FALSE;
			hBlitDC->S1ColorKeyRGB.RGBEnable = FALSE;
		}
	    //ROP and Compositor
...
)
{
	...
	{
		int Err = -EINVAL;
		CSGFXOBJ_Bitmap_t *S1Img = NULL, *DImg = NULL;
		S1Img = &Src1Para->Data.Bitmap;
		DImg  = &DesPara->Bitmap;
		//S0 Control
		{
			...
			// if S0 Using Default Color then S0ColorType should be ARGB565 or A0
			hBlitDC->S0ColorType  = CSGFXOBJ_COLOR_TYPE_A0;
			memcpy(&hBlitDC->S0DefaultColor, &Src0Para->Data.Color, sizeof(CSGFXOBJ_Color_t));
			hBlitDC->S0ClutEnable     = FALSE;
			hBlitDC->S0ColorKeyRGB.RGBEnable = FALSE;
		}
