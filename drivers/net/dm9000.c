...
extern void virgo_irq_enable (unsigned int);
extern void virgo_irq_disable (unsigned int);
extern void virgo_irq_ack (unsigned int);
...
#define DM9K_IRQ         	VIRGO_IRQ_ETH
#define DM9KS_MIN_IO             VA_ETH_BASE
#define DM9KS_MAX_IO            (VA_ETH_BASE + 0x70)
...
static void write_srom_word(board_info_t *, int,u16);
...
static void outb(u8 data, unsigned long ioport)
{
        iowrite8(data, (void *) ioport);
        return;

}

static void outl(u32 data, unsigned long ioport)
{
        iowrite32(data, (void *) ioport);
        return;
}

static void outw(u16 data, unsigned long ioport)
{
        iowrite16(data, (void *) ioport);
        return;

}

static u8 inb(unsigned long ioport)
{
  return  ioread8((void *) ioport);
}

static u16 inw(unsigned long ioport)
{
  return  ioread16((void *) ioport);
}
static u32 inl(unsigned long ioport)
{
  return ioread32((void *) ioport);
}
...
/*        printk("in dmfe_prbe() functions iobase=0x%x$$$$$$$$$$$$$$\n",iobase); */
	...
	do {
		...
		if (id_val == DM9KS_ID || id_val == DM9010_ID) {
	         printk("id_val equal to DM9KS_ID!!!!!");		
		...
		//		return -ENODEV;
...
#if 0   // because in our board there is not SROM
			...
			for (i=0; i<6; i++)
			   { dev->dev_addr[i] = db->srom[i];
                 mactest+= dev->dev_addr[i];    
			}
			printk("MAC=%x:%x:%x:%x:%x:%x\n",dev->dev_addr[0],dev->dev_addr[1],dev->dev_addr[2],dev->dev_addr[3],dev->dev_addr[4],dev->dev_addr[5]);
#endif 	
#if 1
#ifdef VIRGO_ETH_MAC_FROM_FLASH
			//Read mac address from flash
            // printk("mac_v=%x,mac_p=%x",VA_MAC_BASE,PA_MAC_BASE);
			mac_pointer= ioremap(PA_MAC_BASE,8);
			if (mac_pointer==NULL) {
               printk("Mac address is Wrong!!");
			}
			for (i=0;i<6;i++) {
			   dev->dev_addr[i] = inb(mac_pointer+i);
			 //  printk("addr[%d]=%02x",i,dev->dev_addr[i]);
			   mactest+= dev->dev_addr[i];   
		   }
#endif
#endif
        ...
        printk("iobase=%x, dm9000_found=%d\n",iobase,dm9000_found);
...
//	if (request_irq(dev->irq,&dmfe_interrupt,SA_SHIRQ |SA_INTERRUPT,dev->name,dev))
if (request_irq(dev->irq,&dmfe_interrupt,SA_SHIRQ,dev->name,dev))
		return -EAGAIN; 

    //printk("dmfe_interrupt= %x",(unsigned long)dmfe_interrupt);
...
#endif
{
	...
	u8 reg_save;
	disable_irq(irq);
    // printk("in eth interrupt handler\n");
	...
	spin_unlock(&db->lock);
    enable_irq(29);
  // printk("ok intertupt handler exit\n");
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,5,0)
	return IRQ_HANDLED;
#endif
}
...
static void write_srom_word(board_info_t *db, int offset, u16 value)
{
	iow(db, DM9KS_EPAR, offset);
	...
	iow(db, DM9KS_EPCR, 0x32);
	while (ior(db, DM9KS_EPCR) & 0x20);
	iow(db, DM9KS_EPCR, 0x0);

/*
     iow(db, DM9KS_EPCR, 0x20);
     udelay(200);
     iow(db, DM9KS_EPCR, 0x0);
*/
	return;
